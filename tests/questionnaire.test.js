...
test("答题页使用单屏布局并只让长题文区域滚动", () => {
  ...
  assert.match(template, /class="reset-button"[^>]*aria-label="重新开始当前测试"[^>]*><text class="reset-icon">↻<\/text><\/button>/);
  ...
  assert.match(styles, /\.reset-icon\s*\{[\s\S]*?font-size:\s*38rpx;[\s\S]*?line-height:\s*1;/);
});

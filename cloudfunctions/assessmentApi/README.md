...
4. 云函数首次调用时会创建 `assessment_sessions`、`assessment_results` 和 `assessment_consents` 集合。
5. 在云开发控制台将三个集合的客户端权限设为“所有用户不可读写”。小程序只通过云函数访问数据，云函数会按服务端 OpenID 隔离记录。
...
用户同意云端同步后，`assessment_results` 会保存逐题答案和分数；拒绝同意时不会上传答案。`assessment_consents` 只保存同意版本、范围和时间，不保存姓名、头像、手机号、邮箱、年龄或性别。
